...
-- These functions need to have their search_path set to prevent security vulnerabilities
...
-- 5. Fix update_device_commands_updated_at function
DROP FUNCTION IF EXISTS public.update_device_commands_updated_at();
...
DROP FUNCTION IF EXISTS public.check_schedule_conflicts_by_screen_type(uuid, text, timestamptz, timestamptz, uuid);
...
-- 7. Fix notify_playlist_items_change function
DROP FUNCTION IF EXISTS public.notify_playlist_items_change();
...
DROP FUNCTION IF EXISTS public.get_device_alert_summary(uuid);
...
DROP FUNCTION IF EXISTS public.setup_admin_profile(uuid, text, text, uuid);
...
-- 11. Fix notify_media_change function
DROP FUNCTION IF EXISTS public.notify_media_change();
...
DROP FUNCTION IF EXISTS public.generate_pairing_code();
...
DROP FUNCTION IF EXISTS public.generate_device_token();
...
-- 14. Fix notify_playlist_change function
DROP FUNCTION IF EXISTS public.notify_playlist_change();
