...
export async function GET(request: NextRequest) {
  try {
    ...
    const schedules = (allSchedules || []).filter(schedule => {
      ...
      console.log(`🔍 Schedule "${schedule.name}": matches=${matches} (screen_type=${screen.screen_type}, target_types=${JSON.stringify(schedule.target_screen_types)}, screen_type_match=${screenTypeMatch}, all_screens=${allScreensMatch})`)
      return matches
    })
    ...
    const syncData = {
      ...
      power_schedule: screen.power_schedule_enabled ? {
        ...
        on_time: screen.power_on_time?.substring(0, 5) || '06:00', // Strip seconds: "07:00:00" -> "07:00"
        off_time: screen.power_off_time?.substring(0, 5) || '22:00', // Strip seconds: "19:00:00" -> "19:00"
