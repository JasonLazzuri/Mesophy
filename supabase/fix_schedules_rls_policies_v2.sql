-- Fix RLS policies for schedules table with correct user_role enum values
...
-- Policy for deleting schedules: Users can delete schedules they created or super_admins can delete any in org
CREATE POLICY "schedules_delete_policy" ON schedules
    ...
    USING (
        ...
        AND (
            ...
            created_by = auth.uid()
            OR
            -- User is super_admin (using correct enum value)
            EXISTS (
                SELECT 1 FROM user_profiles 
                WHERE id = auth.uid() 
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'screen_schedules') THEN
        ...
        DROP POLICY IF EXISTS "screen_schedules_select_policy" ON screen_schedules;
        DROP POLICY IF EXISTS "screen_schedules_insert_policy" ON screen_schedules;
        DROP POLICY IF EXISTS "screen_schedules_update_policy" ON screen_schedules;
        DROP POLICY IF EXISTS "screen_schedules_delete_policy" ON screen_schedules;
...
-- Verification: Test that the policies work
SELECT 
    'RLS policies updated successfully' as status,
    COUNT(*) as total_policies_on_schedules
FROM pg_policies 
