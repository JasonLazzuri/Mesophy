-- Migration: Implement Screen-Type-Specific Scheduling
-- This replaces the global schedule concept with screen-type targeting

-- Phase 1: Add target_screen_types column to schedules table
...
-- Phase 2: Migrate existing global schedules to target specific screen types
-- First, let's identify existing global schedules and their intended screen types based on their names
UPDATE schedules 
...
-- For any remaining global schedules, set them to target all screen types
UPDATE schedules 
SET target_screen_types = ARRAY['menu_board', 'employee_board', 'promotional', 'outdoor_sign']::screen_type[]
WHERE screen_id IS NULL 
  AND target_screen_types IS NULL;

-- Phase 3: For specific screen schedules, set target_screen_types based on their assigned screen
UPDATE schedules 
...
-- Phase 4: Create new conflict detection function that's screen-type aware
...
-- Phase 5: Create a helper function to get all available screen types
CREATE OR REPLACE FUNCTION get_screen_types()
RETURNS screen_type[] AS $$
BEGIN
    RETURN ARRAY['menu_board', 'employee_board', 'promotional', 'outdoor_sign']::screen_type[];
END;
$$ LANGUAGE plpgsql;
...
-- Phase 7: Update RLS policies to account for target_screen_types (if needed)
-- The existing RLS policies should still work, but we can optimize them later

-- Phase 8: Verification queries to check migration success
-- These are just for verification, not part of the migration
/*
-- Verify screen type distribution
SELECT 
    'Screen Type Distribution' as check_name,
    target_screen_types,
...
-- Verify no schedules are left without proper screen type targeting
SELECT 
    'Schedules without target types' as check_name,
    COUNT(*) as count
FROM schedules 
WHERE is_active = true 
    AND target_screen_types IS NULL;

-- Test the new conflict function
SELECT 
    'Testing new conflict function' as check_name,
    *
FROM check_schedule_conflicts(
    ...
    ARRAY[1,2,3,4,5],
    1,
    ARRAY['employee_board']::screen_type[]
);
*/